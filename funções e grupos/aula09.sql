...
select count (*) 'Total de Notas Fiscais' 
from nf_cliente;

SELECT COUNT(DISTINCT 'Total de Notas Fiscais')
from prod_nf;
go
...
FROM nf_cliente
GROUP BY vl_tot_nf
HAVING MAX(vl_tot_nf) > 150.00;

select 
	SUM (vl_tot_nf) ' Somatória das notas fiscais',
	AVG (vl_tot_nf) 'Média das notas',
	MAX (vl_tot_nf) 'Valor Máximo' , 
	MIN(vl_tot_nf)  'Valor Minimo'
from nf_cliente;
...
select 
	SUM (vl_tot_desc) ' Somatória das notas fiscais',
	AVG (vl_tot_desc) 'Média das notas',
	MAX (vl_tot_desc) 'Valor Máximo' , 
	MIN(vl_tot_desc)  'Valor Minimo'
from nf_cliente;
